...
def findallconnectedneighborgeoms(geomid,generic_laminate,geom_dict,layerdef):
    '''find all the connected shapes'''
    connectedgeomids = [geomid]
    testids= [geomid]
    while len(testids)>0:
        result = findconnectedneighborgeoms(testids.pop(),generic_laminate,geom_dict,layerdef)
