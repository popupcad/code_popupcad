...
def getjoints(geoms):
    ...
    l4 = popupcad.algorithms.points.distance_of_lines(
        lines, [
            10 * tolerance, 0])
    l5 = popupcad.algorithms.points.distance_of_lines(
        lines, [
            10 * tolerance, 10 * tolerance])
    l6 = popupcad.algorithms.points.distance_of_lines(
        lines, [
            0, 10 * tolerance])
    l7 = popupcad.algorithms.points.distance_of_lines(
        lines, [
...
#    jj = numpy.searchsorted(m3,m2)
    ...
    [indeces_to_orig[item].append(ii)
     for ii, item in enumerate(index_to_unique)]
    ...
    for segments in indeces_to_orig:
        if len(segments) > 1:
            ...
            ordered_vertices = popupcad.algorithms.points.order_vertices(
                vertices,
                a[0],
                tolerance=tolerance)
            ...
            for ii in segments:
                for jj, point in enumerate(midpoints):
                    if popupcad.algorithms.points.point_within_line(
                            point,
                            lines[ii],
                            tolerance=tolerance):
                        count[jj] += 1
            newsegments.extend(
                [seg for count_ii, seg in zip(count, segs) if count_ii > 1])
