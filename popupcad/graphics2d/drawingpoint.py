...
class DrawingPoint(qg.QGraphicsEllipseItem,Common):
    radius = 10
    z_below = 100
    z_above = 101
...
#        self.connectedinteractive = connectedinteractive
    ...
    def hoverEnterEvent(self,event):
        super(DrawingPoint,self).hoverEnterEvent(event)
...
#                sibling.setZValue(sibling.z_below)            
    ...
    def hoverLeaveEvent(self,event):
        super(DrawingPoint,self).hoverLeaveEvent(event)
        self.setZValue(self.z_below)
        self.updatestate(self.states.state_neutral)
...
#            self.connectedinteractive.updateshape()
        ...
        return super(DrawingPoint,self).itemChange(change,value)
        
    def mousePressEvent(self,event):
        super(DrawingPoint,self).mousePressEvent(event)
...
#            self.removefromscene()
    ...
    def mouseReleaseEvent(self,event):
        super(DrawingPoint,self).mouseReleaseEvent(event)    
        self.updatestate(self.states.state_hover)
        self.changed_trigger = False

    def setPos(self,*args):
        super(DrawingPoint,self).setPos(*args)
        self.updategeneric()        
