    ...
    def acceptdata(self):
        ...
        for b in self.radiobuttons:
            if b.isChecked():
                option = b.uservalue
        operationlinks = [
            self.le1.currentRefs()[0],
            self.le2.currentRefs()[0],
            self.le3.currentRefs()[0]]
...
class CutOperation2(Operation):
    ...
    def buildeditdialog(self, design):
        operationindeces = [
            [design.operation_index(l[0]), l[1]] for l in self.operation_links]
        dialog = Dialog(
            self.keepout_types,
            self.valuenames,
            self.defaults,
    ...
    def generate(self, design):
        ...
        sheet = design.op_from_ref(
            ...
            self.operation_links[0][1]].csg
        supported_device = design.op_from_ref(
            self.operation_links[1][0]).output[
            self.operation_links[1][1]].csg
        device = design.op_from_ref(
            self.operation_links[2][0]).output[
            self.operation_links[2][1]].csg

        if self.keepout_type == self.keepout_types.laser_keepout:
            keepout = popupcad.algorithms.keepout.laserkeepout(
                supported_device)
            ...
            keepout = popupcad.algorithms.keepout.millflipkeepout(
                supported_device)
            keepout2 = popupcad.algorithms.keepout.laserkeepout(device)
