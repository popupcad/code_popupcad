...
#        layout.addWidget(qg.QLabel('Sketch'))
    ...
    def acceptdata(self):
        ...
        subopref = subopid, jj
        return self.sketch().id, self.subdesign().id, subopref, transformtype_x, transformtype_y, self.sb.value(
        ...
        for layerout, layerin in zip(
            ...
            for geom in sketch.operationgeometry:
                if not geom.is_construction():
                    for designgeom in designgeometry.layer_sequence[
                            layerin].geoms:
                        try:
                            newgeoms.append(
