...
def initialize(program):
    ...
    supportactions.append({'text':'Custom Support','kwargs':{'icon':Icon('customsupport'),'triggered':lambda:program.editor.newoperation(manufacturing.customsupport3.CustomSupport3)}})
    
    other = []
    other.append({'text':'Keep-outs','kwargs':{'icon':Icon('firstpass'),'triggered':lambda:program.editor.newoperation(manufacturing.keepout3.KeepOut3)}})
#    other.append({'text':'Cuts','kwargs':{'icon':Icon('firstpass'),'triggered':lambda:program.editor.newoperation(manufacturing.cutop2.CutOperation2)}})
    other.append({'text':'Identify Rigid Bodies','kwargs':{'triggered':lambda:program.editor.newoperation(manufacturing.identifyrigidbodies2.IdentifyRigidBodies2)}})
...
#        manufacturingactions.append({'text':'Tool Clearance','kwargs':{'triggered':lambda:program.editor.newoperation(ToolClearance2)}})
    manufacturingactions.append({'text':'Removability','kwargs':{'icon':Icon('removability'),'triggered':lambda:program.editor.newoperation(manufacturing.removability2.Removability2)}})
    manufacturingactions.append({'text':'Identify Bodies','kwargs':{'icon':Icon('identifybodies'),'triggered':lambda:program.editor.newoperation(manufacturing.identifybodies2.IdentifyBodies2)}})
